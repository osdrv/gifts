class GiftsController < ApplicationController
  ...
  def index
    if (login = params[:login])
      ...
      if !@user
        render :status => 404, :text => "User with such name not found"
      end
      if (@is_me = @user.is_me?(self.current_user))
        @gifts = @user.gifts
      else 
        @is_friend = @user.is_friend?(self.current_user)
        @is_anonimous = self.current_user ? false : true
        @gifts = @user.public_gifts(self.current_user)
      end
    else
      @gifts = Gift.get_recent_wishes(null)
    end
